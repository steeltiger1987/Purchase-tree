@extends('admin.layout')
	@section('body')
		<h3 class="page-title">Edit Help  Management</h3>
			<!-- page layout -->
			<div class="page-bar">
				<ul class="page-breadcrumb">
					...
					<li>
						<a href="{{URL::route('admin.helpCreating.edit', $help->id)}}">Edit Help</a>
					</li>
				</ul>
			</div>
			<div class="row">
					...
					<div class="portlet box blue">
						<div class="portlet-title">
							...
							<form  class="form-horizontal" id="addCategoryFiledForm" method="POST" action="{{URL::route('admin.helpCreating.store')}}" enctype="multipart/form-data">
                                <input type="hidden" name="createID" value="{{$help->id}}">
							    <div class="form-body">
                                     @foreach ([
                                        ...
                                        'content' =>'Content'
                                                  ...
                                                  <div class="col-md-7 col-sm-7 col-xs-12">
                                                      <select name="category" class="form-control" onchange="onChangeCategory()" id="category">
                                                           ...
                                                           @foreach($category as $categories)
                                                            @if($categories->id == $help->category_id)
                                                  ...
                                                  <div class="col-md-7 col-sm-7 col-xs-12">
                                                      <select name="subcategory" class="form-control" id="subcategory">
                                                        ...
                                                        @foreach($subcategory as $subcategories)
                                                            @if($subcategories->id == $help->subcategory_id)
                                                                <option value = "{{$subcategories->id}}" selected>{{$subcategories->subcategoryname}}</option>
                                                            @else
                                         ...
                                         @elseif($key==='title')
                                            <div class="form-group" id="countryname">
                                                 <label class="col-md-3 col-sm-3 col-xs-12 control-label">{{ Form::label($key, $value) }}</label>
                                                  <div class="col-md-7 col-sm-7 col-xs-12">
                                                      <textarea class="form-control" name="title">{{$help->title}}</textarea>
                                                  </div>
                                            </div>
                                          @else
                                            <div class="form-group" id="countryname">
                                                 <label class="col-md-3 col-sm-3 col-xs-12 control-label">{{ Form::label($key, $value) }}</label>
                                                  <div class="col-md-7 col-sm-7 col-xs-12">
                                                      <textarea class="form-control" name="content" id="content">{{$help->content}}</textarea>
